...
public class TenantSetupService : ITenantSetupService
{
    ...
    /// <summary>
    /// Seeds measurement fields and templates for a new tenant.
    /// NOTE: This method does NOT manage its own transaction — it must be called
    /// from within an existing transaction (e.g. ProvisionShopAsync).
    /// </summary>
    public async Task SetupNewTenantAsync(Guid tenantId, Guid shopId, Guid createdBy, CancellationToken ct = default)
    {
        ...
        // SaveChanges is called here — commit/rollback is handled by the outer transaction.
